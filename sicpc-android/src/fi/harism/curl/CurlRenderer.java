...
public class CurlRenderer implements GLSurfaceView.Renderer {
	...
	float translateXDis = 0;
	float translateYDis = 0;
	float storedTranslateXDis, storedTranslateYDis = 0;
	float maxHorizontalDistance = 0l;
	float maxVerticalDistance = 0l;
	PointF translateStartP;
	...
	public void exitPinch(){
		scaleFactor = 1f;
		translateXDis = translateYDis = storedTranslateXDis = storedTranslateYDis = 0f;
	}
	/**
	...
	public void resetStoredTranslateDis() {
		storedTranslateXDis = storedTranslateYDis = 0;
	}

	public void storeTranslateDis() {
		storedTranslateXDis = translateXDis;
		storedTranslateYDis = translateYDis;
	}



	public void setTranslateStartP(PointF translateStartP) {
		this.translateStartP = translateStartP;
	}

	public void setTrasnlateDistance(float x, float y) {
		translateXDis = x + storedTranslateXDis;
		translateYDis = y + storedTranslateYDis;

		maxHorizontalDistance = (this.mViewportWidth * (this.scaleFactor) - this.mViewportWidth) / 2;
		maxVerticalDistance = (this.mViewportHeight * (this.scaleFactor) - this.mViewportHeight) / 2;
		
		maxHorizontalDistance = (maxHorizontalDistance * this.mViewRect.width())/(this.mViewportWidth * this.scaleFactor);
		maxVerticalDistance = (maxVerticalDistance * this.mViewRect.height())/(this.mViewportHeight * this.scaleFactor);

//		Log.d(TAG, "maxHorizontalDistance: " + maxHorizontalDistance
//				+ " maxVerticalDistance: " + maxVerticalDistance);
		if (Math.abs(translateXDis) > Math.abs(maxHorizontalDistance)) {
			if (translateXDis < 0) {
				Log.d(TAG, "translateXDis < 0");
				translateXDis = -Math.abs(maxHorizontalDistance);
			} else {
				translateXDis = Math.abs(maxHorizontalDistance);
			}
		}
		if (Math.abs(translateYDis) > Math.abs(maxVerticalDistance)) {
			if (translateYDis < 0) {
				translateYDis = -Math.abs(maxVerticalDistance);
			} else {
				translateYDis = Math.abs(maxVerticalDistance);
			}
		}
	...
	// private float currentSizeX = this.mPageRectRight.
	@Override
...
//		Log.d(TAG, "translateXDis: " + translateXDis + " translateYDis: "
//				+ translateYDis);
//
//		Log.d(TAG, "mPageRectRight: " + mPageRectRight);
		if (translateXDis == 0 && translateYDis == 0
				&& USE_PERSPECTIVE_PROJECTION) {
			gl.glTranslatef(0, 0, -6f);
		}

		gl.glScalef(this.scaleFactor, this.scaleFactor, 0);

		if (translateXDis != 0 || translateYDis != 0) {
			// gl.glTranslatef(xDis, yDis, 0);
			gl.glTranslatef(translateXDis, translateYDis, 0);
...
//		Log.d(TAG, "mViewportWidth: " + mViewportWidth + " mViewportHeight: "
//				+ mViewportHeight + " ratio: " + ratio);
		mViewRect.top = 1.0f;
	...
	public float translateX(float x) {
		return mViewRect.left + (mViewRect.width() * x / mViewportWidth);

	}

	public float translateY(float y) {
		return mViewRect.top - (-mViewRect.height() * y / mViewportHeight);
